...
module BookBot
  ...
  RSpec.describe 'A book' do
    ...
    let ( :book ) { BookBot::Book.new(data) }
    ...
    it 'can have a prologue' do
      expect(book.sections).to include(prologue)
    end

    it 'has chapters' do
      expect(book.sections).to include(chapter_1)
      expect(book.sections).to include(chapter_2)
    end

    it 'can have an epilogue' do
      expect(book.sections).to include(epilogue)
    end

    it 'creates a text file' do
      expect(File.file?("output/book.txt")).to be_truthy
    end
